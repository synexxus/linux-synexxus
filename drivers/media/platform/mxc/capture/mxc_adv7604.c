...
*/
	...
	state.sen.streamcap.timeperframe.denominator = DEFAULT_FPS;
	state.sen.streamcap.timeperframe.numerator = 1;
	state.std_id = V4L2_STD_ALL;
	//video_idx = ADV7180_NOT_LOCKED;
	state.sen.pix.width = 640;
	state.sen.pix.height = 480;
	state.sen.pix.pixelformat = V4L2_PIX_FMT_UYVY;  /* YUV422 */
	state.sen.streamcap.capability = V4L2_MODE_HIGHQUALITY | V4L2_CAP_TIMERFRAME;
	...
	*/
	for (i=1; i < ADV7604_PAGE_MAX; ++i){
		if(!(BIT(i) & state->info->page_mask))
			continue;
		state->i2c_clients[i] = adv7604_dummy_client(sd, state->pdata.i2c_addresses[i],0xf2 + i);
...
module_i2c_driver(adv7605_i2c_driver);
