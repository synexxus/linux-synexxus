...
/*!
 * @file drivers/media/video/mxc/capture/mxc_v4l2_genericcapture.c
 *
 * @brief Mxc Video For Linux 2 driver For Generic Data Capture
 *
...
#include "mxc_v4l2_gcapture.h"
#include "ipu_prp_sw.h"

#define init_MUTEX(sem)         sema_init(sem, 1)

static struct platform_device_id imx_v4l2_gdevtype[] = {	
	{
...
MODULE_DEVICE_TABLE(platform, imx_v4l2_gdevtype);

static const struct of_device_id mxc_v4l2_gdt_ids[] = {
	{
		.compatible = "fsl,imx6q-v4l2-gcapture",
		.data = &imx_v4l2_gdevtype[IMX6_V4L2],
	}, {
		/* sentinel */
	}
};
MODULE_DEVICE_TABLE(of, mxc_v4l2_gdt_ids);
...
static struct v4l2_output mxc_genericcapture_outputs[MXC_V4L2_CAPTURE_NUM_OUTPUTS] = {
	{
	 .index = 0,
	 .name = "DISP3 BG",
...
static struct v4l2_input mxc_capture_inputs[MXC_V4L2_CAPTURE_NUM_INPUTS] = {
	 ...
	 */
	typedef enum {
		PI7VD9008_NTSC = 0,		/*!< Locked on (M) NTSC video signal. */
		PI7VD9008_PAL=1,			/*!< (B, D, G, H, I)PAL video signal. */
		PI7VD9008_NV=2,			/*!< Not Valid. */
		PI7VD9008_NTSC_4_43=3,		/*!< (4.43)NTSC video signal. */
		PI7VD9008_PAL_M=4,		/*!< (M) PAL video signal. */
		PI7VD9008_PAL_CN=5,		/*!< (CN) PAL video signal. */
		PI7VD9008_PAL_60=6,		/*!< (60) PAL video signal. */
		PI7VD9008_NOT_LOCKED=7,		/*!< Not locked on a signal. || ALSO AUTO DETECT IF SETTING STANDARD SELECTION */
		PI7VD9008_FMT_IDX_SIZE
	} video_fmt_idx;

	#define TV_NOT_LOCKED PI7VD9008_FMT_IDX_SIZE
...
#define TV_STD_MAX		(PI7VD9008_NOT_LOCKED + 1)
...
static int mxc_v4l2_g_fmt(cam_data *cam, struct v4l2_format *f)
{
	...
	case V4L2_BUF_TYPE_VIDEO_OUTPUT:
		pr_debug("   type is V4L2_BUF_TYPE_VIDEO_OUTPUT\n");
...
static int mxc_v4l2_s_fmt(cam_data *cam, struct v4l2_format *f)
{
	...
	switch (f->type) {
	case V4L2_BUF_TYPE_VIDEO_OUTPUT:
		pr_debug("   type=V4L2_BUF_TYPE_VIDEO_OUTPUT\n");
	case V4L2_BUF_TYPE_VIDEO_CAPTURE:
...
static int mxc_v4l2_g_ctrl(cam_data *cam, struct v4l2_control *c)
{
	...
	case V4L2_CID_BRIGHTNESS:
		if (cam->sensor) {
			...
			pr_err("mxc_v4l_g_ctrl: ERROR: v4l2 capture: slave not found!\n");
			status = -ENODEV;
		}
		break;
	case V4L2_CID_HUE:
	...
	case V4L2_CID_BLACK_LEVEL:
		...
		break;
	default:
		pr_err("mxc_v4l2_g_ctrl: default: ERROR: v4l2 capture: unsupported ioctrl!\n");
	}
...
static int mxc_v4l2_s_ctrl(cam_data *cam, struct v4l2_control *c)
{
...
#if defined(USE_ADVANCED_PI7VD9008)
	struct PIV7D9008_sensor_data *sensor_data;
#else
	...
	case V4L2_CID_HUE:
		if (cam->sensor) {
			...
			pr_err("mxc_v4l_s_ctrl: ERROR: v4l2 capture: slave not found!\n");
			ret = -ENODEV;
		}
		break;
	case V4L2_CID_CONTRAST:
	...
	case V4L2_CID_SATURATION:
		if (cam->sensor) {
			...
			pr_err("mxc_v4l2_s_ctrl: ERROR: v4l2 capture: slave not found!\n");
			ret = -ENODEV;
		}
		break;
	case V4L2_CID_RED_BALANCE:
...
static int mxc_v4l2_s_param(cam_data *cam, struct v4l2_streamparm *parm)
{
	...
	printk(KERN_INFO "%s:      Current capabilities are %x\n", __func__,
			currentparm.parm.capture.capability);
	printk(KERN_INFO "%s:      Current capturemode is %d  change to %d\n", __func__,
			currentparm.parm.capture.capturemode,
			parm->parm.capture.capturemode);
	printk(KERN_INFO "%s:      Current framerate is %d  change to %d\n", __func__,
			current_fps, parm_fps);

	/* This will change any camera settings needed. */
	err = vidioc_int_s_parm(cam->sensor, parm);
	if (err) {
		printk(KERN_INFO "%s:   vidioc_int_s_parm returned an error %d\n",
			__func__, err);
		goto exit;
	}
	...
	printk(KERN_INFO "%s: g_fmt_cap returned: %dx%d\n",__func__,
			cam_fmt.fmt.pix.width, cam_fmt.fmt.pix.height);
	 ...
	 */
	printk(KERN_INFO "%s: device_type %d\n",__func__,cam->device_type);
	if(cam->crop_bounds.width == 720 && cam->crop_bounds.height == 625){
		// PAL
		cam->crop_current.width = 720;
		cam->crop_current.height = 576;
	} else if(cam->crop_bounds.width == 720 && cam->crop_bounds.height == 525){
		// NTSC
		cam->crop_current.width = 720;
		cam->crop_current.height = 480;
	} else if (cam->device_type != 1) {
	...
	printk(KERN_INFO "%s: Calling ipu_csi_set_window_pos(%d,%d)\n",__func__,cam->crop_current.left, cam->crop_current.top);
	ipu_csi_set_window_pos(cam->ipu, cam->crop_current.left,
			       cam->crop_current.top,
			       cam->csi);
	printk(KERN_INFO "%s: Calling ipu_csi_init_interface\n",__func__);
...
static int mxc_v4l2_s_std(cam_data *cam, v4l2_std_id e)
{
	...
	if((e & V4L2_STD_PAL) || (e == 1)){
		/* PAL Standards */
		printk(KERN_INFO "%s: Setting standard to PAL %Lx by request %Lx\n", __func__, V4L2_STD_PAL, e);
		cam->standard.id = V4L2_STD_PAL;
		video_index = PI7VD9008_PAL;
	}else if(e & V4L2_STD_PAL_M){
		printk(KERN_INFO "%s: Setting standard to PAL-M %Lx by request %Lx\n", __func__, V4L2_STD_PAL_M, e);
		cam->standard.id = PIV7D9008_PAL_M;		
	}else if(e & (V4L2_STD_PAL_N | V4L2_STD_PAL_Nc)){
		printk(KERN_INFO "%s: Setting standard to PAL-N %Lx by request %Lx\n", __func__, (V4L2_STD_PAL_N | V4L2_STD_PAL_Nc), e);
		cam->standard.id = PIV7D9008_PAL_CN;		
	}else if(e & (V4L2_STD_PAL_60)){
		printk(KERN_INFO "%s: Setting standard to PAL60 %Lx by request %Lx\n", __func__, V4L2_STD_PAL_60, e);
		cam->standard.id = PIV7D9008_PAL_60;		
	}else if(e & (V4L2_STD_NTSC) || (e == 0)){
		printk(KERN_INFO "%s: Setting standard to NTSC-M %Lx by request %Lx\n", __func__, V4L2_STD_NTSC, e);
		cam->standard.id = PIV7D9008_NTSC;		
	} else if(e & (PIV7D9008_NTSC_4_43)){
		printk(KERN_INFO "%s: Setting standard to NTSC-4.43 %Lx by request %Lx\n", __func__, PIV7D9008_NTSC_4_43, e);
		cam->standard.id = PIV7D9008_NTSC_4_43;		
	} else {
		cam->standard.id = V4L2_STD_ALL;
		//video_index = TV_NOT_LOCKED;
		video_index = PIV7D9008_NOT_LOCKED;
		printk(KERN_INFO "%s: WARNING : unrecognized std! %Lx (PAL=%Lx, NTSC=%Lx)\n", __func__,
...
static int mxc_v4l_dqueue(cam_data *cam, struct v4l2_buffer *buf)
{
	...
	cam->frame[frame->index].buffer.field = cam->device_type ? V4L2_FIELD_INTERLACED : V4L2_FIELD_NONE;
	...
	/* Only Process the image further if it is necessary! */
	if(cam->vid_msb != DEFAULT_HW_CSI_CAPTURE_MSB){
		vidioc_int_priv_process_buf(cam->sensor, buf);
	}
...
static int mxc_v4l_open(struct file *file)
{
	...
	int err = 0;
#if defined(USE_ADVANCED_PI7VD9008)
	struct PIV7D9008_sensor_data *sensor;
#else
	struct sensor_data *sensor;
#endif

	printk(KERN_INFO "%s: \n",__func__);
	printk(KERN_INFO "%s: My name is %s\n",__func__, dev->name);

	if (!cam) {
		pr_err("%s: ERROR: v4l2 capture: Internal error, ""cam_data not found!\n", __func__);
		return -EBADF;
	}

	if (cam->sensor == NULL || cam->sensor->type != v4l2_int_type_slave) {
		pr_err("%s: ERROR: v4l2 capture: slave not found!\n", __func__);
		return -EAGAIN;
	}
	...
	printk(KERN_INFO "%s: We have a slave!\n",__func__);
	...
	if (cam->open_count++ == 0) {
		...
		printk(KERN_INFO "%s: Setting up _g_fmt_cap\n",__func__);
		cam_fmt.type = V4L2_BUF_TYPE_VIDEO_CAPTURE;
		vidioc_int_g_fmt_cap(cam->sensor, &cam_fmt);
		printk(KERN_INFO "%s: Returned from vidioc_int_g_fmt_cap\n",__func__);
		...
		cam->crop_current.left = 0;		//adjusted on 02/27/2015
		cam->crop_current.top = 0;
		cam->crop_current.width = cam_fmt.fmt.pix.width;
		cam->crop_current.height = cam_fmt.fmt.pix.height;

		printk(KERN_INFO "End of %s: v2f pix widthxheight %d x %d\n",
			__func__,
			cam->v2f.fmt.pix.width, cam->v2f.fmt.pix.height);
		printk(KERN_INFO "End of %s: crop_bounds widthxheight %d x %d\n",
			__func__,
			cam->crop_bounds.width, cam->crop_bounds.height);
		printk(KERN_INFO "End of %s: crop_defrect widthxheight %d x %d\n",
			__func__,
			cam->crop_defrect.width, cam->crop_defrect.height);
		printk(KERN_INFO "End of %s: crop_current widthxheight %d x %d\n",
			__func__,
			cam->crop_current.width, cam->crop_current.height);
		...
		ipu_csi_set_window_size(cam->ipu, cam->crop_current.width,
					...
					cam->csi);
		printk(KERN_INFO "%s: Calling ipu_csi_set_window\n",__func__);
		ipu_csi_set_window_pos(cam->ipu, cam->crop_current.left,
					cam->crop_current.top,
					cam->csi);
		printk(KERN_INFO "%s: Calling ipu_csi_init_interface\n",__func__);
		ipu_csi_init_interface(cam->ipu, cam->crop_bounds.width,
					...
					csi_param);
		printk(KERN_INFO "%s: Preparing clk call!\n",__func__);
		clk_prepare_enable(sensor->sensor_clk);
		printk(KERN_INFO "%s: Returned from clk call!\n",__func__);
		vidioc_int_s_power(cam->sensor, 1);
		vidioc_int_init(cam->sensor);
		printk(KERN_INFO "%s: Preparing dev_init call!\n",__func__);
		vidioc_int_dev_init(cam->sensor);
	}

	file->private_data = dev;
	printk(KERN_INFO "%s: Preparing to finalize this function\n",__func__);
oops:
	up(&cam->busy_lock);
	return err;
}
			    ...
			    unsigned int ioctlnr, void *arg)
{
	...
	case VIDIOC_REQBUFS: {
		...
		if ((req->type != V4L2_BUF_TYPE_VIDEO_CAPTURE) && (req->type != V4L2_BUF_TYPE_VIDEO_OUTPUT)) {
			pr_err("ERROR: v4l2 capture: VIDIOC_REQBUFS: "
			       "wrong buffer type %d\n", req->type);
			retval = -EINVAL;
			break;
	...
	case VIDIOC_QBUF: {
		...
		//printk(KERN_DEBUG "%s:   case VIDIOC_QBUF\n", __func__);

		spin_lock_irqsave(&cam->queue_int_lock, lock_flags);
		if ((cam->frame[index].buffer.flags & 0x7) == V4L2_BUF_FLAG_MAPPED) {
			cam->frame[index].buffer.flags |= V4L2_BUF_FLAG_QUEUED;
			list_add_tail(&cam->frame[index].queue,&cam->ready_q);	// The Buffer is added to the ready_q
	...
	case VIDIOC_DQBUF: {
		struct v4l2_buffer *buf = arg;
		//printk(KERN_DEBUG "%s:   case VIDIOC_DQBUF\n", __func__);
	...
	case VIDIOC_STREAMON: {
		printk(KERN_DEBUG "%s:   case VIDIOC_STREAMON\n", __func__);
	...
	case VIDIOC_STREAMOFF: {
		printk(KERN_INFO "%s:   case VIDIOC_STREAMOFF\n", __func__);
	...
	case VIDIOC_CROPCAP: {
		...
		if (cap->type == V4L2_BUF_TYPE_VIDEO_OUTPUT) {
			printk(KERN_INFO "%s: V4L2_BUF_TYPE_VIDEO_OUTPUT\n",__func__);
			// don't error for mxc_v4l2_output.out test!
		} else if (cap->type != V4L2_BUF_TYPE_VIDEO_CAPTURE &&
	...
	case VIDIOC_G_PARM: {
		...
		if (cam->sensor)
			...
			pr_err("%s: ERROR: VIDIOC_G_PARM: slave not found!\n", __func__);
			retval = -ENODEV;
		}
		break;
	}
	...
	case VIDIOC_S_PARM:  {
		...
		if (cam->sensor)
			...
			pr_err("%s: ERROR: VIDIOC_S_PARM: slave not found!\n", __func__);
			retval = -ENODEV;
		}
		break;
	}
	...
	case VIDIOC_G_STD: {
		...
		if (cam->sensor)
			...
			pr_err("%s: ERROR: VIDIOC_G_STD: slave not found!\n", __func__);
			retval = -ENODEV;
		}
		break;
	}
	...
	case VIDIOC_S_STD: {
		...
		printk(KERN_INFO "%s: VIDIOC_S_STD\n",__func__);
	...
	case VIDIOC_ENUMOUTPUT: {
		...
		if (output->index >= MXC_V4L2_CAPTURE_NUM_OUTPUTS) {
			pr_err("%s: ERROR: VIDIOC_ENUMOUTPUT: slave not found!\n", __func__);
			retval = -EINVAL;
			break;
		}
	...
	case VIDIOC_ENUM_FMT: {
		...
		if (cam->sensor)
			...
			pr_err("mxc_v4l_do_ioctl: ERROR: v4l2 capture: slave not found!\n");
			retval = -ENODEV;
		}
		break;
	}
...
static int mxc_mmap(struct file *file, struct vm_area_struct *vma)
{
	...
	printk(KERN_INFO "%s: \n",__func__);
	printk(KERN_INFO "%s:        pgoff=0x%lx, start=0x%lx, end=0x%lx\n",__func__,
...
{
	const struct of_device_id *of_id = of_match_device(mxc_v4l2_gdt_ids, &pdev->dev);
	struct device_node *np = pdev->dev.of_node;
	int ipu_id, csi_id, mclk_source,vid_msb;
	int ret = 0;
	...
	/*
	 * DEFAULT Value: 19
	 * 	Any other value will result in a need to shift the buffers to intercept data
 	 *	correctly.  All other values require GATED mode with VSYNC/HSYNC active
	 *	DEFAULT MSB: DATA19->DATA12 based on IMX6DQIEC datasheets
	 */
	ret = of_property_read_u32(np, "vid_msb", &vid_msb);
	if (ret) {
		dev_err(&pdev->dev, "Video MSB location missing or invalid\n");
		return ret;
	}
	...
	printk(KERN_INFO "%s: Completed Initial checks\n",__func__);
	...
	*(cam->video_dev) = mxc_v4l_template;
	...
	cam->vid_msb = vid_msb;				//! Value to determine post processing of buffered data
	...
	sprintf(cam->self->name, "mxc_v4l2_cap%d on ipu %d", cam->csi, cam->ipu_id);
...
/* ******************************************************************************************
	DRIVER FUNCTIONS FOR LINUX KERNEL
 * **************************************************************************************** */
/*!
 ...
 *                to probe
 *
 *	TODO CONVERT TO SUBDEVICE?
 *	Registers a V4L2 INT-DEVICE
 *
...
static int mxc_v4l2_master_attach(struct v4l2_int_device *slave)
{
	...
	int i;
#if defined(USE_ADVANCED_PI7VD9008)
	struct PIV7D9008_sensor_data *sdata = slave->priv;
#else
	struct sensor_data *sdata = slave->priv;
#endif

	printk(KERN_INFO "\t%s: \t mxc_v4l2_master_attch function\n", __func__);
	printk(KERN_INFO "\t%s: \t slave.name = %s\n", __func__, slave->name);
	printk(KERN_INFO "\t%s: \t master.name = %s\n", __func__, slave->u.slave->master->name);
	...
	/* Used to detect TV in (type 1) vs. camera (type 0)
		TODO: This may be irrelavant for generic capture
	 */
	...
	printk(KERN_INFO "\t%s: \t master.name = %s attached to slave.name %s on IPU%dCSI%d\n", __func__, slave->u.slave->master->name, slave->name,cam->ipu_id+1, cam->csi);
	return 0;
}
...
MODULE_AUTHOR("Freescale Semiconductor, Inc. & Synexxus, Inc.");
MODULE_VERSION("1.1");
