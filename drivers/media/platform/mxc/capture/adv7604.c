...
static s32 adv7604_generic_i2c_byte_read(struct i2c_client *client, u8 addr, u8 reg){
	...
	printk(KERN_DEBUG "%s: read original addr=%x\n", __func__, temp_addr);
...
static s32 adv7604_generic_i2c_byte_write(struct i2c_client *client, u8 addr, u8 reg, u8 val){
	...
	printk(KERN_DEBUG "%s: write original addr=%x\n", __func__, temp_addr);
			 ...
			 const struct i2c_device_id *id)
{
	...
	printk(KERN_DEBUG "DBG sensor data is at %p\n", &adv7604_data);
	...
	printk(KERN_DEBUG "%s Requesting reset pin\n", __func__);
	/* request reset pin */
	...
	printk(KERN_DEBUG "%s devm_clk_get mclk\n", __func__);
	...
	printk(KERN_DEBUG "%s devm_clk_get mclk_source\n", __func__);
	...
	printk(KERN_DEBUG "%s devm_clk_get ipu_id\n", __func__);
	...
	printk(KERN_DEBUG "%s devm_clk_get csi_id\n", __func__);
	...
	printk(KERN_DEBUG "%s all values read.  Preparing struct\n", __func__);
...
//	adv7604_data.streamcap.capability = V4L2_MODE_HIGHQUALITY |
	...
	printk(KERN_DEBUG "%s Struct prepared.  Attempting reset\n", __func__);
	adv7604_reset();
	...
	printk(KERN_DEBUG "%s ADV7604 found.  Attempting device enable\n", __func__);
	adv7604_power_down(1);
	...
	if (!IS_ERR(gpr)) {
		if (of_machine_is_compatible("fsl,imx6q")) {
			...
			printk(KERN_DEBUG "%s: syscon_regmap_lookup: mask:%x %x\n", __func__,mask,val);
	...
	printk(KERN_DEBUG "%s IPU/CSI attached.  Allocating Device\n", __func__);
	clk_disable_unprepare(adv7604_data.sensor_clk);

	adv7604_int_device.priv = &adv7604_data;

	printk(KERN_DEBUG "%s Allocated to adv7604_int_device. Attempting device registration\n", __func__);
	retval = v4l2_int_device_register(&adv7604_int_device);

	printk(KERN_DEBUG "%s ADV7604 Registered Successfully\n", __func__);
