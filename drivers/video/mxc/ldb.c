		    ...
		    struct mxc_dispdrv_setting *setting)
{
	...
	if(1) pr_info("%s Found Device Name: %s\n", __func__,dev->init_name);
	...
	if (!dts) {
		...
		fb_videomode_from_videomode(&chan->vm, &m);				// This contains the timing needed!

		if(1) dev_err(dev, "\tldb: timings found = %u\n",dts->num_timings);
		if(1) dev_err(dev, "\tldb: Native Mode found = %u\n",dts->native_mode);
		if(1) pr_info("%s: ret=%d, %dx%d\n", __func__, ret, m.xres, m.yres);
		if(1) pr_info("%s:r=%d, x=%d, y=%d, p=%d, l=%d, r=%d, upper=%d, lower=%d, h=%d, v=%d\n",
			__func__, m.refresh, m.xres, m.yres, m.pixclock, m.left_margin, m.right_margin, 
			m.upper_margin, m.lower_margin, m.hsync_len, m.vsync_len);

		//ret = fb_find_mode(&setting->fbi->var, setting->fbi, setting->dft_mode_str,
		INIT_LIST_HEAD(&fbi->modelist);
		//fb_add_videomode(&m, &fbi->modelist);
		//fb_videomode_to_var(&fbi->var, &m);	
		...
		for(i=0; i < dts->num_timings; i++){
			...
			if(ret == -EINVAL){
				...
				if (i==dts->native_mode) {
					fb_videomode_to_var(&fbi->var, &fb_vm);		//Make the Native mode in DTS the video mode

					/* update fbi mode */
					setting->fbi->mode = (struct fb_videomode *)&fb_vm;
					fb_videomode_to_var(&setting->fbi->var, (struct fb_videomode *)&fb_vm);
				}
			}	
		}
	...
	/*
	mode = fb_find_nearest_mode(&m, &fbi->modelist);	// Now let's obtain the nearest mode
	if (!mode) {
		pr_err("%s: could not find mode in modelist\n", __func__);
		printk(KERN_INFO "Using native mode\n");
		if(1) pr_info("%s Using Native modeline\n", __func__);
	} else {
		printk(KERN_INFO "Using command line mode\n");
		if(1) pr_info("%s Using kernel boot command modeline\n", __func__);
		// Let's choose the command line mode
		fb_videomode_to_var(&setting->fbi->var, mode);

		// update fbi mode 
		setting->fbi->mode = (struct fb_videomode *)mode;
	}
	*/
