...
void report_single_touch(void)
{
	...
	input_report_abs(crtouch->input_dev, ABS_PRESSURE, 1.0);
...
void report_multi_touch(void)
{
	...
	//BBB input_report_key(crtouch->input_dev, ABS_MT_TRACKING_ID, 0);
	/* input_report_abs(crtouch->input_dev, ABS_MT_TOUCH_MAJOR, 1);
	input_report_abs(crtouch->input_dev, ABS_MT_POSITION_X, crtouch->x1);
	input_report_abs(crtouch->input_dev, ABS_MT_POSITION_Y, crtouch->y1);
	input_mt_sync(crtouch->input_dev);*/

	//BBB input_report_key(crtouch->input_dev, ABS_MT_TRACKING_ID, 1);
	/*input_report_abs(crtouch->input_dev, ABS_MT_TOUCH_MAJOR, 1);
...
{
	//BBB input_event(crtouch->input_dev, EV_ABS, ABS_MT_TOUCH_MAJOR, 0);
	//BBB input_mt_sync(crtouch->input_dev);
	input_report_abs(crtouch->input_dev, ABS_PRESSURE, 0.0);
...
void free_two_touch(void){
	...
	input_mt_sync(crtouch->input_dev);*/
	//input_sync(crtouch->input_dev);
	printk("SYNEXXUS-CRTOUCH-FREETWO-REMOVED\n");
				 ...
				 const struct i2c_device_id *id)
{
	...
	//BBB __set_bit(KEY_A, crtouch->input_dev->keybit);
	...
	//BBB input_set_abs_params(crtouch->input_dev, ABS_MT_POSITION_X,
	//BBB 			XMIN, xmax, 0, 0);
	//BBB input_set_abs_params(crtouch->input_dev, ABS_MT_POSITION_Y,
	//BBB 			YMIN, ymax, 0, 0);
	//BBB input_set_abs_params(crtouch->input_dev, ABS_MT_TOUCH_MAJOR, 0, 1, 0, 0);
	//BBB input_set_abs_params(crtouch->input_dev, ABS_MT_TRACKING_ID, 0, 1, 0, 0);
	...
	if (result < 0) {
		...
		//goto err_free_pinIrq;
		// SHOULD NO LONGER NEED To free gpio_pin
		goto err_unr_cdev;
	}

	/* SEMI converted st1232 Code*/
	/*result = request_threaded_irq(&client->dev, client->irq,
                        NULL, crtouch_irq, IRQF_ONESHOT, client->name, crtouch);
		if (result) {
	 		printk(KERN_DEBUG "unable to request IRQ\n");
...
static int crtouch_remove(struct i2c_client *client)
{
	...
	free_irq(client->irq, crtouch_irq);
...
static const struct of_device_id of_crtouch_dt_ids[] = {
...
static struct i2c_driver crtouch_fops = {
	...
	.driver = {
		   //.owner = THIS_MODULE,
		   .name = "crtouch_mt",
	   	   .of_match_table = of_crtouch_dt_ids,
...
MODULE_DEVICE_TABLE(of, of_crtouch_dt_ids);
...
module_i2c_driver(crtouch_fops);
